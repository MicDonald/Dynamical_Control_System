...
#pragma omp for
        for (int i=t/update->dt;i>=1;--i){
                ...
                {
                        uv+=tempUv;
                        if(test) std::cout<<"["<<omp_get_thread_num()<<":"<<omp_get_num_threads()<<"]"<<"Kernel("<<ti<<")*ur("<<ii<<")*"<<update->dt<<std::endl;
                }
        }
}
#pragma omp single
{
        if (test) {
                uv*=test;
                std::cout<<"uv*test constant("<<test<<"): "<<uv.size()<<"x1\n"<<uv.transpose()<<std::endl;
        }
		...
		std::cout<<" x:"<<x[i][0]<<" y:"<<x[i][1]<<" z:"<<x[i][2]<<std::endl;
                ...
                std::cout<<"Update ["<<jv<<"]"
                         <<" x: "<<K.bondOrAtom2MatrixDof(jv)[0]
                         <<" y: "<<K.bondOrAtom2MatrixDof(jv)[1]
                         <<" z: "<<K.bondOrAtom2MatrixDof(jv)[2]
                         <<" GID: "<<gid<<std::endl;
                }
